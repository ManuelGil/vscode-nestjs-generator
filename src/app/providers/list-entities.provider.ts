/**
 * @file Tree data provider that discovers files containing ORM entity
 * decorators (TypeORM, Sequelize, MikroORM, Mongoose) and lists their
 * annotation lines as navigable child nodes in the sidebar.
 */
...
/**
 * Discovers workspace files containing ORM-specific decorators and shows
 * matching annotation lines as child nodes. Uses {@link ORMController} to
 * obtain the regex that identifies entity annotations for the configured ORM
 * (TypeORM, Sequelize, MikroORM, or Mongoose).
 *
 * @class
...
export class ListEntitiesProvider implements TreeDataProvider<NodeModel> {
   ...
   *
   * @constructor
   * @param controller - ORM controller providing annotation regex.
   */
  ...
  /**
   * Returns the children for the supplied element, or the top-level entity
  ...
  // Private methods
  /**
   * Scans all workspace files for ORM-specific annotations using the regex
   * provided by {@link ORMController.getAnnotationsRegex}. Files with matches
   * are returned with annotation lines as clickable child nodes.
   *
   * @returns Entity file nodes with their annotation lines as children.
   */
